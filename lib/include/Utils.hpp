...
struct MetaData {
    VecS labels;
    // whether a label is numeric or categorical
    std::vector<bool>  label_type;
    // whether it is a classLabel
    std::vector<bool> isClassLabel;
    // number of levels if categoric,0 if numeric
    std::vector<int> label_levels;
    std::vector<int> number_used;
    VecS Classes;
...
struct DataObject{
    ...
    std::vector<std::vector<std::pair<std::string,int>>> features;
    // store index and class label
    std::map<int,std::string> index_class;
