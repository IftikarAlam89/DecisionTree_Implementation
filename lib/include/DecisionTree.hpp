...
class DecisionTree {
  public:
    ...
    explicit DecisionTree(const DataReader& dr,std::mt19937_64 ran);
    ...
    DataObject createDataObject(const Data& data);
   const Node buildTree(DataObject & rows, MetaData &meta);
    MetaData createMetaObject(const MetaData& data);
    DataObject createDataObjectBag(const Data& data,std::mt19937_64 ran)
